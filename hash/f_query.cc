...
struct RollingHash {
    ...
    array <int, M> query(int l, int r) {
        ...
        array <int, M> h;
        for (int i = 0; i < M; ++i) {
            h[i] = (buf[i][r] - 1ll * buf[i][l] * rhf.pw[i][r - l]) % rhf.mods[i];
...
RollingHashFactory <2> rhf;
vector <RollingHash <2> > rhs(26, RollingHash <2>(rhf));

vector < array <int, 2> > get(int x, int len) {
    vector < array <int, 2> > ret;
    for (int i = 0; i < 26; ++i) {
        ret.push_back(rhs[i].query(x, x + len - 1));
    }
    return ret;
}
